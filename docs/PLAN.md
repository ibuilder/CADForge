...
| **2b — IFC in** | `IfcBackend` over `ifc-lite` ([ADR-0009](adr/0009-ifc-lite-as-the-read-backend.md), measured); project to `ElementRecord` | Import → export → re-import preserves `GlobalId`, relationships, psets on a 5-file corpus; output opens in Bonsai and Revit |
